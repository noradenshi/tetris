...
enum class PieceType { o, s, z, t, l, j, i, all };
...
class Bag {
  ...
  public:
    ...
    void operator=(Bag &rh) { m_queue = rh.m_queue; }
};
...
class BagDrawer {
  ...
  public:
    PieceType nextPiece() { return m_bags[rand() % size].nextPiece(); }

    void operator=(BagDrawer rh) { m_bags = rh.m_bags; }
};
...
class Piece {
  ...
  public:
    ...
    std::array<PieceCell, 4>::iterator begin() { return m_piece.begin(); }
    std::array<PieceCell, 4>::iterator end() { return m_piece.end(); }
};

class PiecePreview : private Piece,
                     public sf::Drawable,
